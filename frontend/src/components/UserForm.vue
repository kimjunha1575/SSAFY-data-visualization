<template>
  <div>
    ...
    <div>
      <button v-on:click="makeGroup">그룹확정</button>
    </div>
    <div>
      ...
      <div v-if="keywordGroups.length">
        <div v-for="(keywordGroup, index) in keywordGroups" v-bind:key="index">
          <div>
            그룹 이름: {{ keywordGroup.groupName }}
          </div>
          <div>
            그룹 키워드: {{ keywordGroup.keywords }}
          </div>
        </div>
      </div>
      <div>
...
function makeGroup() {
  ...
  tempGroupName.value = ""; tempKeywords.value = [];
...
async function sendResultToApi() {
  ...
  await dataLap.post(toApiData.value);
